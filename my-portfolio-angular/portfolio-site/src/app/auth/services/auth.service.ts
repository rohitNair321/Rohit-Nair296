...
export class AuthService {
  ...
  login(payload: LoginRequest): Observable<any> {
    return this.http.post<any>(`${this.apiV1BaseUrl}/login`, payload, this.httpOptions).pipe(
      map((res) => {
        ...
        this.role.set(data.user.role === 'admin' ? 'ADMIN' : 'GUEST');
        this.appService.setRole(data.user.role === 'admin' ? 'ADMIN' : 'GUEST');
        this.localStorageService.setItem('auth_token', data.token);
      })
    );
  }
  ...
  logoutState(): void {
    ...
    this.role.set(null);
    this.token.set(null);
    this.appService.clearRole(); // Clear role from localStorage
    this.appService._profile.set(null);
    this.localStorageService.clear();
  }

  logout(): void {
    this.http.post(`${this.apiV1BaseUrl}/logout`, {}, this.httpOptions).subscribe({
      next: () => {
        this.logoutState();
